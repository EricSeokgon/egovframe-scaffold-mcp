 ...
 *   2) MCP         : src/index.ts 의 TEMPLATES (공식 저장소 직접 조달)
...
async function main() {
  ...
  const { TEMPLATES } = await import(path.join(ROOT, "dist", "index.js"));
  ...
  const catalog = {
    ...
    sources: {
      ...
      mcp: { source: "src/index.ts TEMPLATES", templates: mcpEntries.length },
    },
