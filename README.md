...
| `list_egovframe_components` | 선택 설치 가능한 공통컴포넌트 카탈로그 (M1: 대표 3종) |
| `add_egovframe_components` | 공통컴포넌트 선택 조립 — 의존성 포함 소스·매퍼·JSP 복사, DB DDL·DML 스크립트 복사(`database`), 충돌 시 전체 거부, `dryRun` 미리보기 |
...
1. 공통컴포넌트 선택 설치 — **M2 완료(v0.3.0)**: 카탈로그 기반 조립 파이프라인 동작. 남은 항목: 카탈로그 자동 생성·커버리지 확대(M3), 컴포넌트별 테이블 선별 DDL (설계: [docs/design-components-parameter.md](docs/design-components-parameter.md))
